<?php /* Template Name: Homepage */ ?>

<?php get_header(); ?>

<div id="primary" class="content-area">
<main id="main" class="site-main" role="main">
<section class="hero-block vh-70 mvh-55 p-3 p-sm-0 py-sm-5 position-relative bg-light-t-o-g d-flex align-items-center">
  <div class="container h-100">
    <div class="row d-flex align-items-center h-100">
      <div class="col-md-6 col-lg-6 p-5">
        <svg xmlns="http://www.w3.org/2000/svg" width="320" viewBox="0 0 894.33 150.58"><defs><style>.cls-1{fill:#ffbc00;}</style></defs><title>Asset 3</title><g id="Layer_2" data-name="Layer 2"><g id="Layer_1-2" data-name="Layer 1"><polygon class="cls-1" points="79.17 0 103.63 49.57 158.33 57.52 118.75 96.1 128.09 150.58 79.17 124.86 30.24 150.58 39.58 96.1 0 57.52 54.7 49.57 79.17 0"/><polygon class="cls-1" points="265.17 0 289.63 49.57 344.33 57.52 304.75 96.1 314.09 150.58 265.17 124.86 216.24 150.58 225.58 96.1 186 57.52 240.7 49.57 265.17 0"/><polygon class="cls-1" points="448.17 0 472.63 49.57 527.33 57.52 487.75 96.1 497.09 150.58 448.17 124.86 399.24 150.58 408.58 96.1 369 57.52 423.7 49.57 448.17 0"/><polygon class="cls-1" points="634.17 0 658.63 49.57 713.33 57.52 673.75 96.1 683.09 150.58 634.17 124.86 585.24 150.58 594.58 96.1 555 57.52 609.7 49.57 634.17 0"/><polygon class="cls-1" points="815.17 0 839.63 49.57 894.33 57.52 854.75 96.1 864.09 150.58 815.17 124.86 766.24 150.58 775.58 96.1 736 57.52 790.7 49.57 815.17 0"/></g></g></svg>
        <h1 class="mt-4">The Shipping Software with the most 5-star reviews!</h1>
        <p class="subtitle">Why is ShipStation the leading choice of online sellers? Because no other platform makes shipping faster, easier, and more affordable.</p>
        <div class="hero-btn-container d-flex justify-content-between align-items-center">
          <a href="#" role="button" class="btn btn-primary">Try for Free</a><a href="#">Schedule a Demo</a>
        </div>
      </div>
    </div>
  </div>
</section>
<div class="container logo-stripe d-flex justify-content-around align-items-center flex-wrap">
      <img src="/wp-content/uploads/2021/02/Amazon-Logo.png" alt="Amazon logo">
      <img src="/wp-content/uploads/2021/02/1024px-Shopify_logo_2018.svg_.png" alt="Shopify logo">
      <img src="/wp-content/uploads/2021/02/United-States-Postal-Service-Logo-e1613164517972.png" alt="United States  Postal Service logo">
      <img src="/wp-content/uploads/2021/02/ebay_logo.png" alt="Ebay logo">
      <img src="/wp-content/uploads/2021/02/FedEx-Logo-PNG-Transparent.png" alt="FedEx logo">
</div>
<div class="container">
    <div class="row d-flex justify-content-center align-items-start section-title">
        <h2 class="col-sm-12 col-md-6 m-0 p-0">The fastest, easiest way to get products to your customers</h2>
        <p class="col-sm-12 col-md-5 border-left">ShipStation includes an unparalleled buying and shipping experience with every order you send. All the tools you need to exceed your customers' expectations</p>
    </div>
</div>

<div class="container card-group">
  <div class="card">
    <svg height="70pt" viewBox="0 -31 512.00026 512" width="70pt" xmlns="http://www.w3.org/2000/svg"><circle style="fill: #F7BEC052" cx="300" cy="220" r="200"/><path style="fill: #E7625F" d="m164.960938 300.003906h.023437c.019531 0 .039063-.003906.058594-.003906h271.957031c6.695312 0 12.582031-4.441406 14.421875-10.878906l60-210c1.292969-4.527344.386719-9.394532-2.445313-13.152344-2.835937-3.757812-7.269531-5.96875-11.976562-5.96875h-366.632812l-10.722657-48.253906c-1.527343-6.863282-7.613281-11.746094-14.644531-11.746094h-90c-8.285156 0-15 6.714844-15 15s6.714844 15 15 15h77.96875c1.898438 8.550781 51.3125 230.917969 54.15625 243.710938-15.941406 6.929687-27.125 22.824218-27.125 41.289062 0 24.8125 20.1875 45 45 45h272c8.285156 0 15-6.714844 15-15s-6.714844-15-15-15h-272c-8.269531 0-15-6.730469-15-15 0-8.257812 6.707031-14.976562 14.960938-14.996094zm312.152343-210.003906-51.429687 180h-248.652344l-40-180zm0 0"/><path style="fill: #E7625F" d="m150 405c0 24.8125 20.1875 45 45 45s45-20.1875 45-45-20.1875-45-45-45-45 20.1875-45 45zm45-15c8.269531 0 15 6.730469 15 15s-6.730469 15-15 15-15-6.730469-15-15 6.730469-15 15-15zm0 0"/><path style="fill: #E7625F" d="m362 405c0 24.8125 20.1875 45 45 45s45-20.1875 45-45-20.1875-45-45-45-45 20.1875-45 45zm45-15c8.269531 0 15 6.730469 15 15s-6.730469 15-15 15-15-6.730469-15-15 6.730469-15 15-15zm0 0"/></svg>
    <div class="card-body">
      <h4 class="card-title">Import orders from more places</h4>
      <p class="card-text">Import orders from over 100 marketplaces, cards and stores - more than any competitor</p>
    </div>
  </div>

  <div class="card">
    <svg height="70pt" width="70pt" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 392.6 392.6" style="enable-background:new 0 0 392.6 392.6;" xml:space="preserve">
    <g>
	  <g>
    <circle style="fill: #c8b1f552" cx="196" cy="196" r="162"/><path style="fill: #896FBC;" d="M387.1,99.4c-0.4-0.8-0.4-1.2-1.2-2c-0.4-0.8-1.2-1.2-2.4-1.6l-91.6-47.6L199.1,0.6c-1.6-0.8-3.6-0.8-5.2,0l-93.2,47.6
			l-92,47.6c-1.2,0.4-2.4,1.6-2.8,2.8c-0.4,0.8-0.4,1.2-0.8,2v0.4v190.4c0,2,1.2,4,3.2,5.2l185.2,94.8c0,0,0.4,0,0.4,0.4
			c0,0,0.4,0,0.4,0.4c0.8,0.4,1.2,0.4,2,0.4s1.2,0,2-0.4c0,0,0.4,0,0.4-0.4c0,0,0.4,0,0.4-0.4l185.2-94.8c2-1.2,3.2-3.2,3.2-5.2V101
			C387.5,100.2,387.5,99.8,387.1,99.4z M196.3,12.2l90,46.4l82.8,42.4l-56.8,29.2L139.5,41.4L196.3,12.2z M271.5,163.8l20-10.4
			l13.2-6.8v24.8l-10.8-4.8c-2.4-1.2-5.2-0.4-6.8,1.6L271.9,187v-23.2H271.5z M106.3,58.6l20.4-10.4l172.8,88.4l-33.6,17.6
			L93.1,65.4L106.3,58.6z M190.7,377.4l-174-89.6V110.6l84.4,43.2l89.6,46V377.4z M196.3,189.8l-90-46.4L23.5,101l56.8-29.2
			l172.8,88.8L196.3,189.8z M375.9,288.2l-174,88.8V199.8l58.4-30v33.6c0,3.2,2.4,5.6,5.6,5.6c2,0,3.6-0.8,4.8-2.4l22.4-27.6
			l15.2,6.4c2.8,1.2,6.4,0,7.6-3.2c0.4-0.4,0.4-1.2,0.4-2V141l59.6-30.4V288.2z"/>
	    </g>
      </g>
    </svg>
    <div class="card-body">
      <h4 class="card-title">Instantly rate shop your favorite carriers</h4>
      <p class="card-text">Rate shop and print labels for all carriers - get the best rate based on desired delivery time.</p>
    </div>
  </div>

  <div class="card">
      <svg id="icons" enable-background="new 0 0 64 64" height="70pt" viewBox="0 0 64 64" width="70pt" xmlns="http://www.w3.org/2000/svg"><circle style="fill: #b4f8c852" cx="32" cy="33" r="28"/><path style="fill: #297768;" d="m7.943 54.644c-.008.008-.01.018-.017.025-.046.048-.075.108-.11.166s-.076.111-.1.173c-.003.01-.012.017-.015.026-.016.046-.009.092-.017.138-.014.073-.03.145-.027.219.003.063.021.121.035.183.015.063.025.126.053.187.029.065.074.12.117.178.029.039.045.085.08.12.008.008.018.01.025.018.048.045.108.073.165.108.058.036.111.077.174.101.01.003.017.012.026.015l7.426 2.476c.002.001.004 0 .005.001.1.032.204.051.312.051s.212-.019.312-.051c.002-.001.003 0 .005-.001.009-.003.015-.011.023-.014.104-.037.197-.093.282-.162.032-.025.058-.054.085-.082.055-.056.102-.116.143-.185.022-.039.043-.076.061-.117.012-.025.029-.046.038-.072.013-.038.006-.076.014-.114.015-.067.025-.132.025-.202s-.011-.135-.025-.202c-.008-.038-.001-.076-.014-.114l-2.476-7.425c-.003-.01-.012-.017-.015-.026-.022-.062-.063-.115-.1-.173-.035-.058-.064-.118-.11-.166-.007-.008-.009-.018-.017-.025-.034-.034-.078-.049-.115-.076-.06-.045-.115-.092-.184-.122-.059-.026-.119-.036-.181-.051-.063-.015-.122-.033-.187-.036-.075-.003-.146.014-.221.027-.046.008-.091.001-.137.017-.01.003-.017.012-.026.015-.063.023-.116.064-.174.101-.057.035-.117.063-.164.108-.008.008-.019.01-.026.018l-.678.678c-9.827-10.575-9.603-27.169.688-37.46 6.774-6.774 16.736-9.452 26.006-7.01.023.008.04.025.063.031.215.058.434.086.65.086.434 0 .86-.113 1.245-.334.58-.334.994-.873 1.165-1.518.173-.646.085-1.32-.248-1.898-.301-.521-.773-.897-1.335-1.096-.058-.027-.107-.066-.171-.084-11.011-2.948-22.855.23-30.907 8.282-6.044 6.044-9.372 14.079-9.372 22.627-.001 8.215 3.08 15.95 8.683 21.904zm5.221-2.392 1.331 3.993-3.994-1.331zm-11.167-20.248c0-8.014 3.12-15.548 8.786-21.214 7.528-7.527 18.592-10.508 28.897-7.788.025.009.045.028.071.035.129.035.236.117.302.231.067.116.085.252.049.384-.045.172-.164.262-.23.3-.104.06-.223.073-.34.053-.014-.005-.024-.015-.038-.019-9.979-2.658-20.71.218-28.004 7.512-11.069 11.069-11.294 28.933-.688 40.287l-.708.708c-5.225-5.576-8.097-12.809-8.097-20.489z"/><path style="fill: #297768;" d="m55.32 10.094.737-.737c.014-.014.019-.032.031-.047.057-.063.103-.136.143-.215.018-.034.039-.063.052-.099.004-.011.012-.02.016-.03.016-.046.009-.092.017-.138.014-.073.03-.145.027-.219-.003-.063-.021-.121-.035-.183-.015-.063-.025-.126-.053-.187-.029-.065-.074-.12-.117-.178-.029-.039-.045-.085-.08-.12-.008-.008-.019-.01-.026-.018-.047-.045-.107-.073-.164-.108-.058-.036-.111-.077-.174-.101-.01-.003-.017-.012-.026-.015l-7.426-2.475c-.036-.012-.071-.006-.107-.014-.069-.015-.137-.026-.209-.025-.072-.001-.14.011-.208.025-.036.008-.072.002-.108.014-.021.007-.036.023-.057.032-.067.027-.127.065-.188.107-.049.034-.097.065-.138.106-.045.045-.079.097-.115.15-.039.057-.074.111-.101.175-.009.022-.026.038-.034.061-.013.038-.006.076-.014.114-.015.067-.025.132-.025.202s.011.135.025.202c.008.038.001.076.014.114l2.476 7.425c.004.011.012.02.016.03.013.035.034.064.052.099.04.079.086.151.143.215.013.015.018.033.031.047.016.016.037.021.054.035.065.059.137.104.216.144.032.016.062.035.095.048.109.04.224.066.343.066.104 0 .211-.017.316-.052.01-.003.017-.012.026-.015.063-.023.116-.064.174-.1.057-.036.117-.064.165-.109.008-.008.018-.01.025-.018l.677-.677c9.827 10.574 9.602 27.17-.688 37.46-6.772 6.772-16.73 9.451-25.998 7.011-.025-.009-.045-.028-.071-.035-1.341-.352-2.708.441-3.061 1.769-.173.646-.085 1.321.25 1.899.3.519.771.894 1.33 1.092.058.028.108.068.174.086 2.716.726 5.481 1.078 8.229 1.078 8.396 0 16.615-3.298 22.682-9.364 6.044-6.044 9.372-14.079 9.372-22.627-.001-8.211-3.082-15.946-8.685-21.9zm-4.483 1.655-1.331-3.994 3.994 1.331zm2.38 41.462c-7.528 7.527-18.593 10.506-28.895 7.789-.025-.01-.045-.029-.072-.036-.129-.035-.236-.117-.303-.231-.067-.115-.085-.251-.049-.383.066-.25.315-.398.57-.354.014.004.023.015.037.019 2.459.655 4.962.975 7.449.975 7.608 0 15.059-2.988 20.555-8.485 11.07-11.07 11.295-28.933.688-40.288l.708-.708c5.225 5.577 8.097 12.81 8.097 20.489.001 8.013-3.119 15.547-8.785 21.213z"/><path style="fill: #297768;" d="m47.995 45v-23.005c0-.13-.026-.26-.077-.382-.051-.123-.124-.234-.217-.326l-4-4.001c-.092-.093-.203-.166-.326-.217-.122-.051-.252-.077-.382-.077h-21.997c-.13 0-.26.026-.382.077-.123.051-.234.124-.326.217l-3.999 4.001c-.093.092-.166.203-.217.326-.051.122-.077.252-.077.382v23.005c0 .553.447 1 1 1h30c.553 0 1-.447 1-1zm-5.416-26.008 2.002 2.003h-8.805l-.5-2.003zm-11.807 0h2.443l.5 2.003h-3.452zm-.777 4.003h4.001v2.621l-1.702-.564c-.204-.068-.427-.067-.633.001l-1.666.559zm-8.585-4.003h7.3l-.509 2.003h-8.793zm24.585 25.008h-28v-21.005h10v4.006c0 .073.026.138.042.207.008.037.001.074.014.111.003.01.011.017.014.026.02.052.048.096.075.143.032.058.066.112.108.161.034.039.069.072.108.105.054.046.111.083.173.116.041.021.078.044.121.061.105.039.216.064.331.066.005 0 .009.003.014.003h.002s.001 0 .002 0c.105 0 .213-.017.318-.052l2.664-.895 2.7.896c.051.017.101.01.151.018.056.01.105.033.163.033.016 0 .027-.008.043-.009.066-.003.127-.022.191-.038.063-.016.128-.025.186-.053.055-.024.098-.064.146-.099.056-.039.113-.073.16-.123.039-.041.063-.093.096-.142.04-.061.081-.119.107-.188.005-.013.016-.021.02-.034.018-.053.011-.104.02-.157.009-.054.031-.102.031-.157v-4.006h9.999v21.006z"/><path style="fill: #297768;" d="m28.996 36.999h-8c-.553 0-1 .447-1 1s.447 1 1 1h8c.553 0 1-.447 1-1s-.447-1-1-1z"/><path style="fill: #297768;" d="m24.996 39.999h-4c-.553 0-1 .447-1 1s.447 1 1 1h4c.553 0 1-.447 1-1s-.447-1-1-1z"/><path style="fill: #297768;" d="m32.994 37.999v3c0 .553.447 1 1 1h3c.553 0 1-.447 1-1v-3c0-.553-.447-1-1-1h-3c-.553 0-1 .447-1 1zm2 1h1v1h-1z"/><path style="fill: #297768;" d="m40 41.999h3c.553 0 1-.447 1-1v-3c0-.553-.447-1-1-1h-3c-.553 0-1 .447-1 1v3c0 .553.447 1 1 1zm1-3h1v1h-1z"/>
    </svg>
    <div class="card-body">
      <h4 class="card-title">Make returns easy for you and your customers</h4>
      <p class="card-text">Include a custom self-service portal for fast, hassle-free customer returns.</p>
    </div>
  </div>
</div>

    <div class="text-center text-with-cta">
      <h2>Start your free trial today</h2>
      <p>Start saving time and money! Try our 30-day risk free trial. <br> No credit card required</p>
      <a href="#" role="button" class="btn btn-primary">Try for Free</a>
    </div>
  <section class="container features-list">
    <div class="row d-flex justify-content-center align-items-start img-list">
      <div class="phone-container">
        <img height="644" width="640" class="phone-bottom" src="https://i.ibb.co/581530n/bottom.png" alt="an iPhone with ShipStation open"><img height="644" width="640" class="phone-top" src="https://i.ibb.co/Zz5R82F/top.png" alt="another iPhone with ShipStation open">
      </div>
      <div class="col-sm-12 col-md-5 section-title">
        <h2 class="mt-0 mb-2">Expansive features across desktop and mobile</h2>
        <p class="mb-5">No one connects you to more customers and more sales channels than ShipStation.</p>
            <div class="d-flex col-md-auto justify-content-around align-items-start list-items">
              <div style="min-width: 70px;">
              <svg transform="rotate(10)" height="60pt" width="60pt" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 490 490" style="enable-background:new 0 0 490 490;" xml:space="preserve">
<g>
	<g>
		<circle style="fill: #F7BEC052" cx="250" cy="250" r="243"/><path style="fill: #E7625F" d="M181.837,293.967L20.761,455.041v-50.35c0-5.543-4.494-10.037-10.039-10.037c-5.543,0-10.038,4.494-10.038,10.037v74.582
			v0.012v0.676c0,5.547,4.495,10.039,10.038,10.039h75.271c5.543,0,10.04-4.492,10.04-10.039c0-5.543-4.497-10.039-10.04-10.039
			h-51.72l161.759-161.756c3.923-3.922,3.923-10.277,0-14.199C192.111,290.047,185.757,290.047,181.837,293.967z"/>
		<path style="fill: #E7625F" d="M489.316,479.273v-74.582c0-5.543-4.493-10.037-10.038-10.037c-5.545,0-10.04,4.494-10.04,10.037v50.35L308.167,293.969
			c-3.921-3.918-10.275-3.918-14.195,0c-3.923,3.924-3.923,10.279,0,14.199l161.755,161.754h-51.719
			c-5.544,0-10.04,4.496-10.04,10.039c0,5.547,4.496,10.039,10.04,10.039h75.27c5.545,0,10.038-4.492,10.038-10.039v-0.676V479.273z
			"/>
	</g>
	<g>
		<path style="fill: #E7625F" d="M181.837,196.033L20.761,34.963v50.346c0,5.545-4.494,10.037-10.039,10.037c-5.543,0-10.038-4.492-10.038-10.037v-74.58
			v-0.01v-0.678C0.684,4.496,5.179,0,10.722,0h75.271c5.543,0,10.04,4.496,10.04,10.041c0,5.543-4.497,10.037-10.04,10.037h-51.72
			l161.759,161.76c3.923,3.92,3.923,10.275,0,14.195C192.111,199.955,185.757,199.955,181.837,196.033z"/>
		<path style="fill: #E7625F" d="M489.316,10.729v74.58c0,5.545-4.493,10.037-10.038,10.037c-5.545,0-10.04-4.492-10.04-10.037V34.963L308.167,196.029
			c-3.921,3.924-10.275,3.924-14.195,0c-3.923-3.918-3.923-10.273,0-14.195L455.727,20.078h-51.719
			c-5.544,0-10.04-4.494-10.04-10.037c0-5.545,4.496-10.041,10.04-10.041h75.27c5.545,0,10.038,4.496,10.038,10.041v0.678V10.729z"
			/>
	</g>
</g>
</svg>
              </div>
              <div style="max-width: 26ch;">
                <h4>Full-integrated Connections</h4>
                <p>Our shipping solutions seamlessly integrate with all of the major eCommerce shopping carts and platforms enabling you to easily manage and ship your online orders all in one place.</p>
              </div>
            </div>

            <div class="d-flex col-md-auto justify-content-around align-items-start list-items">
              <div style="min-width: 70px;">
              <svg height="70pt" width="70pt" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
<g>
	<g>
		<g>
			<circle style="fill: #c8b1f552" cx="250" cy="250" r="245"/><path style="fill: #896FBC;" d="M119.467,349.867c-14.138,0-25.6,11.462-25.6,25.6s11.461,25.6,25.6,25.6s25.6-11.461,25.6-25.6
				C145.051,361.335,133.599,349.882,119.467,349.867z M119.467,384c-4.713,0-8.533-3.82-8.533-8.533s3.82-8.533,8.533-8.533
				s8.533,3.82,8.533,8.533C127.995,380.178,124.178,383.995,119.467,384z"/>
			<path style="fill: #896FBC;" d="M392.533,349.867c-14.138,0-25.6,11.462-25.6,25.6s11.462,25.6,25.6,25.6c14.138,0,25.6-11.461,25.6-25.6
				C418.118,361.335,406.665,349.882,392.533,349.867z M392.533,384c-4.713,0-8.533-3.82-8.533-8.533s3.821-8.533,8.533-8.533
				c4.713,0,8.533,3.82,8.533,8.533C401.062,380.178,397.244,383.995,392.533,384z"/>
			<path style="fill: #896FBC;" d="M505.496,228.4l-58.783-66.2c-4.87-5.484-11.858-8.615-19.192-8.6h-77.654v-51.2c-0.015-14.132-11.468-25.585-25.6-25.6
				H25.6C11.468,76.815,0.015,88.268,0,102.4v221.867c-0.001,2.263,0.898,4.434,2.499,6.035c1.6,1.6,3.771,2.499,6.035,2.499h8.533
				v25.6c0.015,14.132,11.468,25.585,25.6,25.6h17.748c4.206,29.38,29.372,51.2,59.052,51.2s54.846-21.82,59.052-51.2h154.964
				c4.206,29.38,29.372,51.2,59.052,51.2c29.68,0,54.846-21.82,59.052-51.2H486.4c14.132-0.015,25.585-11.468,25.6-25.6V245.417
				C511.994,239.138,509.679,233.082,505.496,228.4z M17.067,315.733V102.4c0.005-4.711,3.822-8.529,8.533-8.533h298.667
				c4.711,0.005,8.529,3.822,8.533,8.533v213.333H17.067z M60.415,366.933H42.667c-4.711-0.005-8.529-3.822-8.533-8.533v-25.6h43.61
				C68.37,341.962,62.279,353.959,60.415,366.933z M119.467,418.133c-23.564,0-42.667-19.103-42.667-42.667
				S95.903,332.8,119.467,332.8s42.667,19.103,42.667,42.667C162.106,399.019,143.019,418.106,119.467,418.133z M204.8,366.933
				h-26.282c-1.864-12.974-7.954-24.972-17.328-34.133h43.61V366.933z M281.6,366.933h-59.733V332.8H281.6V366.933z
				 M333.482,366.933h-34.815V332.8h52.143C341.436,341.962,335.346,353.959,333.482,366.933z M392.533,418.133
				c-23.564,0-42.667-19.103-42.667-42.667s19.102-42.667,42.667-42.667c23.564,0,42.667,19.103,42.667,42.667
				C435.172,399.019,416.086,418.106,392.533,418.133z M494.933,358.4c-0.005,4.711-3.822,8.529-8.533,8.533h-34.815
				c-4.255-29.353-29.391-51.147-59.052-51.2h-42.667V170.667H384V256c0.015,14.132,11.468,25.585,25.6,25.6h85.333V358.4z
				 M494.933,264.533H409.6c-4.711-0.005-8.529-3.822-8.533-8.533v-85.333h26.454c2.45-0.014,4.788,1.024,6.421,2.85l58.804,66.225
				c1.4,1.56,2.178,3.579,2.188,5.675V264.533z"/>
		</g>
	</g>
</g>
</svg>
              </div>
              <div style="max-width: 26ch;">
                <h4>Automated Shipping Tasks</h4>
                <p>ShipStation automation rules allow you to automate product lists, attributes, filters, presets, tags, rules, auto-routing for fulfillment and more.</p>
              </div>
            </div>

            <div class="d-flex col-md-auto justify-content-around align-items-start list-items">
              <div style="min-width: 70px;">
                <svg height="70pt" width="70pt" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink"   x="0px" y="0px"
	                 width="491.582px" height="491.582px" viewBox="0 0 491.582 491.582" style="enable-background:new 0 0 491.582 491.582;"
	                 xml:space="preserve">
                  <g>
	                  <g><circle style="fill: #b4f8c852" cx="250" cy="250" r="230"/>
	                	<path style="fill: #297768" d="M245.791,0C153.799,0,78.957,74.841,78.957,166.833c0,36.967,21.764,93.187,68.493,176.926
	                		c31.887,57.138,63.627,105.4,64.966,107.433l22.941,34.773c2.313,3.507,6.232,5.617,10.434,5.617s8.121-2.11,10.434-5.617
	                		l22.94-34.771c1.326-2.01,32.835-49.855,64.967-107.435c46.729-83.735,68.493-139.955,68.493-176.926
	                		C412.625,74.841,337.783,0,245.791,0z M322.302,331.576c-31.685,56.775-62.696,103.869-64.003,105.848l-12.508,18.959
	                		l-12.504-18.954c-1.314-1.995-32.563-49.511-64.007-105.853c-43.345-77.676-65.323-133.104-65.323-164.743
	                		C103.957,88.626,167.583,25,245.791,25s141.834,63.626,141.834,141.833C387.625,198.476,365.647,253.902,322.302,331.576z"/>
	                	<path style="fill: #297768" d="M245.791,73.291c-51.005,0-92.5,41.496-92.5,92.5s41.495,92.5,92.5,92.5s92.5-41.496,92.5-92.5
	                		S296.796,73.291,245.791,73.291z M245.791,233.291c-37.22,0-67.5-30.28-67.5-67.5s30.28-67.5,67.5-67.5
	                		c37.221,0,67.5,30.28,67.5,67.5S283.012,233.291,245.791,233.291z"/>
	                  </g>
                  </g>
                </svg>
              </div>
              <div style="max-width: 26ch;">
                <h4>Self-service Tracking Portal</h4>
                <p>Eliminate those "Where's my package?" calls from customers with your own custom tracking page featuring your company brand, colors, and messaging.</p>
                <a href="#" role="button" class="btn btn-primary">See All Features</a>
              </div>
            </div>
      </div>
    </div>
  </section>
    <section class="video-container">
      <iframe width="560" height="315" src="https://www.youtube.com/embed/sQs1Eejphkc" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    </section>
    <div class="text-center text-with-cta">
      <h2>Powering shipping success of all sizes</h2>
      <p>Learn how our customers are using ShipStation to rapidly grow their business, scale their services, and satisfy customers</p>
      <a href="#" role="button" class="btn btn-primary">See Customer Stories</a>
    </div>
    <div class="container logo-stripe d-flex justify-content-around align-items-center flex-wrap">
        <img src="/wp-content/uploads/2021/02/magnolia-logo.png" alt="Magnolia logo">
        <img src="/wp-content/uploads/2021/02/Lollaland-Logo.png" alt="Lollaland logo">
        <img src="/wp-content/uploads/2021/02/Logo_CarlosBakery.png" alt="Carlo's Bakery logo">
        <img src="/wp-content/uploads/2021/02/beardbrand_logo.png" alt="Beardbrand logo">
        <img src="/wp-content/uploads/2021/02/spigen_logo.png" alt="Spigen logo">
    </div>
    <section class="container h-100 need-help-container">
      <div class="row d-flex align-items-center h-100">
        <div class="col-lg-7 p-5">
          <h2 class="mt-3 mb-3">Need More Help?</h2>
          <p>We would love to show you how to save time and money with ShipStation</p>
          <a href="#" role="button" class="btn btn-primary">Schedule a Demo</a>
        </div>
      </div>
    </section>
</main><!-- .site-main -->


</div><!-- .content-area -->

<?php get_footer(); ?>